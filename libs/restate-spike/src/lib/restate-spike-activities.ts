...
/**
 * Side-effect adapter surface for the M2 spike — Restate edition.
 *
 * This interface is intentionally identical to TemporalSpikeActivity so that the
 * same ForgeScenarioService implementation serves both candidates.
 *
 * Architecture:
 * - Workflow: Pure control flow, calls ctx.run() with these functions, stores only IDs in journal
 * - Activities (via ctx.run): Delegate to a DurableExecutionScenarioService which composes Forge services
 * - Forge services: Execute actual builder/repair logic, persist evidence to SQLite
 */
export interface RestateSpikeActivity {
...
export const createRestateSpikeActivities = (
  service: DurableExecutionScenarioService
): RestateSpikeActivity => ({
